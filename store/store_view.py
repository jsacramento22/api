...
from models.store.store_model import StoreModel
...
class StoreView(StoreModel):
    ...
    def insert(self, request):
        ...
        resp_dict['unique_store_hash'] = self.unique_store_hash_string(
            unique_owner_hash)

        res = self.el.insert_el(
            index='shop_store', data=resp_dict)
    ...
    def unique_store_hash_string(self, unique_personal_hash):
        import json
        import hashlib
        ...
        unique_store_hash = hashlib.sha256(block_string).hexdigest()

        return unique_store_hash
