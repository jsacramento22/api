...
from models.store.store_inventory_transaction_model \
    import StoreInventoryTransactionModel
...
class StoreInventoryTransactionView(StoreInventoryTransactionModel):
    ...
    def retrieve(self, request, **kwargs):
        ...
        resp_list = self.el.retrieve_el(
            index='ecom_store_inventory_transaction',
    ...
    def insert(self, request):
        ...
        buyer_hash = data.get('buyer_hash', '')
        if not buyer_hash:
            resp_dict['status'] = 'Error'
            resp_dict['status_desc'] = 'Campo buyer_hash não ' + \
                'especificado.'
            return jsonify(resp_dict), 400

        seller_hash = data.get('seller_hash', '')
        if not seller_hash:
            resp_dict['status'] = 'Error'
            resp_dict['status_desc'] = 'Campo seller_hash não ' + \
                'especificado.'
            return jsonify(resp_dict), 400
        ...
        resp_dict['transaction_hash'] = self.transaction_hash_string(
            unique_inventory_hash)

        res = self.el.insert_el(
            index='ecom_store_inventory_transaction', data=resp_dict)
    ...
    def transaction_hash_string(self, unique_personal_hash):
        import json
        import hashlib
        ...
        transaction_hash = hashlib.sha256(block_string).hexdigest()
