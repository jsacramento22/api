...
from models.store.store_sales_setup_model import StoreSalesSetupModel
...
class StoreSalesSetupView(StoreSalesSetupModel):
    ...
    def insert(self, request):
        ...
        resp_dict['unique_sales_setup_hash'] = \
            self.unique_sales_setup_hash_string(unique_store_hash)

        res = self.el.insert_el(
            index='ecom_store_sales_setup', data=resp_dict)
    ...
    def unique_sales_setup_hash_string(self, unique_personal_hash):
        import json
        import hashlib
        ...
        unique_sales_setup_hash = hashlib.sha256(block_string).hexdigest()

        return unique_sales_setup_hash
