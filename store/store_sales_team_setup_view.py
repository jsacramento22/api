...
from models.store.store_sales_team_setup_model import StoreSalesTeamSetupModel
...
class StoreSalesTeamSetupView(StoreSalesTeamSetupModel):
    ...
    def retrieve(self, request, **kwargs):
        ...
        resp_list = self.el.retrieve_el(
            index='shop_store_team_sales_setup',
    ...
    def insert(self, request):
        ...
        manager_hash = data.get('manager_hash', '')
        if not manager_hash:
            resp_dict['status'] = 'Error'
            resp_dict['status_desc'] = 'Campo manager_hash não ' + \
                'especificado.'
            return jsonify(resp_dict), 400
        ...
        resp_dict['unique_sales_team_hash'] = self.unique_sales_team_hash_string(
            unique_store_hash)

        res = self.el.insert_el(
            index='shop_store_team_sales_setup', data=resp_dict)
    ...
    def unique_sales_team_hash_string(self, unique_personal_hash):
        import json
        import hashlib
        ...
        unique_sales_team_hash = hashlib.sha256(block_string).hexdigest()

        return unique_sales_team_hash
