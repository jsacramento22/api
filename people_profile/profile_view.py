...
from models.people_profile.profile_model import ProfileModel
...
class ProfileView(ProfileModel):
    ...
    def retrieve(self, request, **kwargs):
        ...
        resp_list, status_code = self.retrieve_constructor(
            unique_personal_hash)

        return resp_list, status_code
    ...
    def insert(self, request, **kwargs):
        ...
        data = kwargs if kwargs else request.get_json()
        ...
        if not data.get('unique_personal_hash', ''):
            resp_dict['status'] = 'Error'
            resp_dict['status_desc'] = 'Campo unique_personal_hash não ' + \
                'especificado.'
            return jsonify(resp_dict), 400
        ...
        res = self.el.insert_el(
            index='myid_people_profile', data=resp_dict)
    ...
    def retrieve_constructor(self, unique_personal_hash):
