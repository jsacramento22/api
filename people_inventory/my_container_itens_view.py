...
from models.people_inventory.my_container_itens_model \
    import MyContainerItensModel
...
class MyContainerItensView(MyContainerItensModel):
    ...
    def retrieve(self, request, **kwargs):
        ...
        resp_list = self.el.retrieve_el(
            index='myid_people_inventory_my_container_itens',
    ...
    def insert(self, request):
        ...
        unique_container_hash = data.get('unique_container_hash', '')
        if not unique_container_hash:
            resp_dict['status'] = 'Error'
            resp_dict['status_desc'] = 'Campo unique_container_hash não ' + \
                'especificado.'
            return jsonify(resp_dict), 400
        ...
        res = self.el.insert_el(
            index='myid_people_inventory_my_container_itens', data=resp_dict)
