...
from models.people_inventory.my_pet_record_model import MyPetRecordModel
...
class MyPetRecordView(MyPetRecordModel):
    ...
    def retrieve(self, request, **kwargs):
        ...
        resp_list = self.el.retrieve_el(
            index='myid_people_inventory_my_pet_record',
    ...
    def insert(self, request):
        ...
        unique_pet_hash = data.get('unique_pet_hash', '')
        if not unique_pet_hash:
            resp_dict['status'] = 'Error'
            resp_dict['status_desc'] = 'Campo unique_pet_hash não ' + \
                'especificado.'
            return jsonify(resp_dict), 400
        ...
        res = self.el.insert_el(
            index='myid_people_inventory_my_pet_record', data=resp_dict)
