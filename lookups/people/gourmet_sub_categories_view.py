...
class GourmetSubCategoriesView():
    ...
    def retrieve(self, request, **kwargs):
        ...
        resp_list = self.el.retrieve_el(
            index='lkup_gourmet_sub_categories',
    ...
    def insert(self, request):
        ...
        if not category:
            resp_dict['status'] = 'Error'
            resp_dict['status_desc'] = 'Campo category não ' + \
                'especificado.'
            return jsonify(resp_dict), 400
        ...
        res = self.el.insert_el(
            index='lkup_gourmet_sub_categories', data=resp_dict)
